...
struct convolution_layer *create_convolution_layer(
  ...
  l->weights = malloc(frames * l->kernelShape.dim2 * sizeof(double));
  l->biases = malloc(frames * sizeof(double));
  init_rand_truncated_norm(l->weights, frames * l->kernelShape.dim2, 0.1);
  init_const(l->biases, frames, 0.1);
...
struct gradient create_gradient_convolution_layer(
  ...
  local->weightErrors = calloc(l->outputShape.depth * l->kernelShape.dim2, sizeof(double));
  local->biasErrors = calloc(l->outputShape.depth, sizeof(double));
  g.extra = local;
  return g;
}
...
void feedforward_convolution_layer(struct convolution_layer *l, struct activation a) {
  ...
  for (int outZ = 0, frame = 0; outZ < l->outputShape.dim2; outZ += l->outputShape.dim1, frame++) {
    double *weights = l->weights + frame * l->kernelShape.dim2;
    double bias = l->biases[frame];
...
void backpropogate_to_props_convolution_layer(struct convolution_layer *l, struct activation a, struct gradient g) {
  ...
  for (int outZ = 0, frame = 0; outZ < l->outputShape.dim2; outZ += l->outputShape.dim1, frame++) {
    double *weightErrors = local_g->weightErrors + frame * l->kernelShape.dim2;
    double *biasError = local_g->biasErrors + frame;
    for (int outY = 0, inYInit = yStart; outY < l->outputShape.dim1; outY += l->outputShape.dim0, inYInit += l->inputShape.dim0) {
      for (int outX = 0, inXInit = xStart; outX < l->outputShape.dim0; outX++, inXInit++) {
...
void update_convolution_layer(struct convolution_layer *l, double rate, struct gradient g) {
  ...
  for(int i = 0; i < l->outputShape.depth; i++) {
    double *weights = l->weights + i * l->kernelShape.dim2;
    double *weightErrors = local_g->weightErrors + i * l->kernelShape.dim2;
    l->biases[i] -= rate * local_g->biasErrors[i];
    for(int j = 0; j < l->kernelShape.dim2; j++) {
      weights[j] -= rate * weightErrors[j];
    }
  }
  memset(local_g->weightErrors, 0, l->outputShape.depth * l->kernelShape.dim2 * sizeof(double));
  memset(local_g->biasErrors, 0, l->outputShape.depth * sizeof(double));
}
